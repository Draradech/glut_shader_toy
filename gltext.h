...
GLT_API void _gltUpdateBuffers(GLTtext *text)
{
	...
	GLfloat glyphOffX = 0.0f;
	GLfloat glyphOffY = 0.0f;
	...
	for (i = 0; i < text->_textLength; i++)
	{
        ...
        glyphOffX = glyphWidth < 8 ? 1 : 0;
        glyphOffY = c=='(' || c==')' ? -1 : 0;
#endif

		if (glyph.drawable)
		{
			vertices[vertexElementIndex++] = glyphOffX + glyphX;
			vertices[vertexElementIndex++] = glyphOffY + glyphY;
			vertices[vertexElementIndex++] = glyph.u1;
			vertices[vertexElementIndex++] = glyph.v1;

			vertices[vertexElementIndex++] = glyphOffX + glyphX + glyphWidth;
			vertices[vertexElementIndex++] = glyphOffY + glyphY + glyphHeight;
